...
def get_data(filepath='../../../../sharecost_ms_annex/plot_data_test.pkl'):
	...
	#probabilities as a function of gamma
	data['probabilities_by_gamma'] = {}
	for coeff_no,coeff_internal in enumerate(data['COEFF_ARRAY_INTERNAL_COINS']):
		
		for instance_no in data_multiple_instances:
			temp = data_multiple_instances[instance_no]['coin_flip_biases_array'][coeff_no,]
			#print "temp",temp
			if instance_no==0:
				data_matrix = copy.deepcopy(temp)
			else:
				data_matrix = numpy.vstack((data_matrix,temp))
		# print "data_matrix",data_matrix
		data['probabilities_by_gamma'][coeff_no] = {}
		data['probabilities_by_gamma'][coeff_no]['median'] = numpy.median(data_matrix, axis=0)
		# print "median",data['probabilities_by_gamma'][coeff_no]['median']
		data['probabilities_by_gamma'][coeff_no]['std'] = numpy.std(data_matrix, axis=0)/math.sqrt(data_matrix.shape[1])
		# print "std",data['probabilities_by_gamma'][coeff_no]['std']
...
def plot_probability_gamma(data):
	...
	for coeff_no,coeff_internal in enumerate(data['COEFF_ARRAY_INTERNAL_COINS']):
		xs 		= data['GAMMA_ARRAY']
		ys 		= data['probabilities_by_gamma'][coeff_no]['median']
		ax.plot(xs, ys,label=data['COEFF_ARRAY_INTERNAL_COINS'][coeff_no])
	...
	plt.title('Probability of Conversion (outside AND inside provider\'s market)')
	plt.show()	
